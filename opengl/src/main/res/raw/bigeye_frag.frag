...
vec2 newCoord1(vec2 coord,vec2 eye,float rmax){
    vec2 newCoord = coord;
    //获得当前需要采集的点与眼睛的距离
...
void main(){
    ...
    //变量newCoord与方法newCoord1不能相同名字
    vec2 newCoord = newCoord1(aCoord,left_eye,rmax);

    newCoord = newCoord1(newCoord,right_eye,rmax);
   //  采集到 RGBA 值
